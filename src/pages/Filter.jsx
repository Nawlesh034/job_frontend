...
const Filter = ({ onChange = () => {} }) => {
  ...
  return (
    <div className="flex flex-wrap justify-around items-center bg-white py-3 px-4 gap-3 sm:gap-4">
      {/* Search */}
      ...
      <div className="flex items-center justify-between gap-2 border-l border-gray-300 pl-4 h-full">
        <label className="text-sm text-[#686868] flex items-center gap-2">
          <input type="checkbox" checked={remote} onChange={(e) => { const checked = e.target.checked; setRemote(checked); // Clear location when remote only is selected
            emit({ remote: checked, location: checked ? "" : location }); }} />
          Remote only
        </label>
      </div>
    </div>
