...
const AdminSignup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post("/api/v1/register", { name, email, password, role: "admin" }, { withCredentials: true });
      navigate("/");
    } catch (err) {
  ...
  return (
    <div className="max-w-md mx-auto mt-16 bg-white p-6 rounded-xl shadow">
      ...
      <p className="text-sm mt-3">Already have an account? <Link to="/admin/login" className="text-blue-600">Log in</Link></p>
    </div>
  );
};
